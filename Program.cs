...
internal class Program
{
	...
	private static DirectoryInfo valheimWorlds = new DirectoryInfo($"{gameDir}/worlds");
	private static DirectoryInfo valheimCharacters = new DirectoryInfo($"{gameDir}/characters");
	...
	private static void Main(string[] args)
	{
		...
		while(true)
		{
			...
			if(delta.Minutes >= config.BackupInterval)
			{
				...
				var characters = newDir.CreateSubdirectory("characters");
				var worlds = newDir.CreateSubdirectory("worlds");

				foreach(var item in valheimCharacters.GetFiles())
				{
					item.CopyTo($"{characters.FullName}/steam_autocloud.vdf");
				}

				foreach(var item in valheimCharacters.GetFiles())
				{
					item.CopyTo($"{worlds.FullName}/steam_autocloud.vdf");
				}
